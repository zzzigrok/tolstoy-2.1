...
class HeuristicSyntaxEngine:
    ...
    def __init__(self):
        ...
        self.group4_patterns = [
            ...
            (r"\b(взял[аои]?|возьм\w+)\s+и\s+\w+\b", "Взять - и - Vf (Внезапность)"),
            (r"\bнет[\s-]нет\s+да\s+и\s+\w+\b", "Нет-нет - да - и - Vf"),
            (r"\bтолько\s+и\s+(делает|делал[аои]?),?\s*что\s+\w+\b", "Только и делает, что..."),
        ...
        self.prepositions = {
            "в", "во", "на", "с", "со", "у", "к", "ко", "по", "из", "изо", "за", "от", "ото", 
            "до", "об", "обо", "при", "про", "через", "над", "надо", "под", "подо", "перед", 
            "передо", "между", "для", "без", "безо", "ради", "сквозь", "около", "возле"
        }
        self.pronouns = {
            "я", "меня", "мне", "мной", "мною", "ты", "тебя", "тебе", "тобой", "тобою",
            "он", "его", "ему", "им", "нем", "нём", "она", "ее", "её", "ей", "ею", "ней",
            "оно", "они", "их", "им", "ими", "них", "ними", "мы", "нас", "нам", "нами",
            "вы", "вас", "вам", "вами", "кто", "что", "кого", "чего", "кому", "чему",
            "кем", "чем", "ком", "тот", "та", "то", "те", "того", "той", "тех",
            "эти", "этот", "эта", "это", "этого", "этой", "этим", "этом", "все", "весь",
            "вся", "всё", "всего", "всей", "всем", "всех", "свой", "своя", "свое", "свои",
            "мой", "моя", "мое", "мои", "твой", "твоя", "твое", "твои", "наш", "наша", "наше", "наши"
        }
        self.noun_exceptions = {
            "ответ", "совет", "привет", "пакет", "предмет", "букет", "билет", "портрет", 
            "рассвет", "свет", "проект", "поэт", "сонет", "корсет", "жакет", "банкет", 
            "балет", "атлет", "авторитет", "комитет", "университет", "пистолет", "рулет", 
            "скелет", "завет", "паштет", "визит", "гранит", "аппетит", "алфавит", "дефицит", 
            "кредит", "зенит", "магнит", "паразит", "профицит", "депозит", "вердикт",
            "минут", "маршрут", "уют", "институт", "рекрут", "статут", "дебют", "салют", 
            "атрибут", "приют", "зал", "бал", "стол", "стул", "угол", "узел", "осел", 
            "посол", "козел", "зол", "бокал", "вокзал", "канал", "финал", "сигнал", 
            "идеал", "сокол", "ствол", "кристалл", "металл", "журнал", "капитал", "купол",
            "домом", "днем", "днём", "утром", "вечером", "летом", "зимой", "осенью", "шагом",
            "словом", "делом", "путем", "путём", "глазом", "разом", "часом", "городом",
            "часть", "гость", "кость", "власть", "страсть", "кисть", "пасть", "скатерть", 
            "сеть", "путь", "мать", "дочь", "ночь", "вещь"
        }

    def is_verb(self, word):
        w = word.lower()
        if w in self.praed_list or w in self.conjunctions or w in self.prepositions or w in self.pronouns or w in self.noun_exceptions:
            return False
        if len(w) < 2:
            return False
    ...
    def is_inf(self, word):
        w = word.lower()
        if w in self.pronouns or w in self.noun_exceptions:
            return False
        return bool(re.search(self.inf_endings, w))
